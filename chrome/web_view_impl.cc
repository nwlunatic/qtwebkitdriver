...
Status WebViewImpl::SendMouseClickJSONRequestDeprecated(const std::list<MouseEvent>& events) {
   for (std::list<MouseEvent>::const_iterator it = events.begin();
     ...
     //command.SetString("command", "ProcessWebMouseEvent");
     command.SetString("type", GetAsString(it->type));
     command.SetString("button", GetAsString(it->button));
     command.SetInteger("modifiers", it->modifiers);
     command.SetInteger("x", it->x);
     command.SetInteger("y", it->y);
     command.SetInteger("clickCount", it->click_count);
     Status status = client_->SendCommand("ProcessWebMouseEvent", command);
     //Status status = client_->SendCommandDeprecated(command);
     if (status.IsError())
       return status;
  }
  return Status(kOk);
