...
#include "chrome/test/qtwebkitdriver/chrome/util.h"
...
Status DevToolsClientImpl::SendCommandDeprecated(const base::DictionaryValue& command) {
  if (!socket_->IsConnected())
    return Status(kDisconnected, "not connected to DevTools");

  base::DictionaryValue final_command;
  final_command.MergeDictionary(&command); // command.DeepCopy();
  int command_id = next_id_++;
  final_command.SetInteger("id", command_id);
  std::string message;
  base::JSONWriter::Write(&final_command, &message);
